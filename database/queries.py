...
def get_coordinates(address):
    ...
    # Fix: Return None instead of a hardcoded default location
    return None, None 
...
def create_booking(name, phone, pickup, urgency, oxygen, icu, nurse, note):
    ...
    # Fix: Safely handle unresolvable locations
    ...
    return True, fetch_one("SELECT * FROM bookings WHERE phone = ? ORDER BY id DESC", (phone,))
...
def register_customer(name, phone, password):
    ...
    execute("INSERT INTO customers (full_name, phone, password, is_registered) VALUES (?, ?, ?, 1)", (name, phone, hashed_password))
...
def verify_login(role, identifier, password):
    ...
    # First, get the user by their identifier only
    if role == 'Manager':
    ...
    if user:
        # Fix: Check against the secure hash
        if check_password_hash(user['password'], password):
            return True, user
        # Temporary fallback for your existing test accounts with plaintext passwords
...
def get_all_drivers(): return fetch_all("SELECT * FROM drivers")
def get_pending_bookings(): return fetch_all("SELECT * FROM bookings WHERE status = 'pending' ORDER BY id DESC")
def get_available_sina_drivers(): return fetch_all("SELECT * FROM drivers WHERE is_available = 1 AND status = 'active'")
def get_all_suppliers(): return fetch_all("SELECT * FROM suppliers WHERE status = 'active'")
...
def dispatch_mission(booking_id, source, driver_id=None, supplier_id=None):
    ...
    if source == 'sina':
        ...
        execute("INSERT INTO trips (booking_id, driver_id, trip_source) VALUES (?, ?, 'sina')", (booking_id, driver_id))
    else:
        execute("INSERT INTO trips (booking_id, supplier_id, trip_source) VALUES (?, ?, 'external')", (booking_id, supplier_id))
    return True

def get_live_map_data():
    drivers = fetch_all("SELECT full_name, is_available, last_lat, last_lng FROM drivers WHERE last_lat != 0.0")
    pending = fetch_all("SELECT customer_name, urgency, pickup_lat, pickup_lng FROM bookings WHERE status = 'pending' AND pickup_lat != 0.0")
    active_trips = fetch_all("""
        SELECT d.full_name as driver_name, d.last_lat, d.last_lng, b.pickup_lat, b.pickup_lng 
        FROM trips t JOIN drivers d ON t.driver_id = d.id JOIN bookings b ON t.booking_id = b.id
...
def get_assigned_trip_for_driver(driver_id):
    return fetch_one("""
        ...
        ORDER BY t.id DESC LIMIT 1""", (driver_id,))
...
def get_customer_active_status(phone):
    return fetch_one("""
        SELECT b.id as booking_id, b.status as booking_status, b.pickup_location, b.pickup_lat, b.pickup_lng, b.discount_applied,
        ...
        FROM bookings b LEFT JOIN trips t ON b.id = t.booking_id LEFT JOIN drivers d ON t.driver_id = d.id LEFT JOIN suppliers s ON t.supplier_id = s.id
        WHERE b.phone = ? AND b.status != 'completed' AND b.status != 'cancelled_by_customer'
...
def get_completed_missions():
    return fetch_all("""
        SELECT t.*, b.customer_name, b.discount_applied, b.payment_status, b.id as b_id, s.company_name, s.commission_rate
        FROM trips t JOIN bookings b ON t.booking_id = b.id LEFT JOIN suppliers s ON t.supplier_id = s.id
        WHERE t.trip_status = 'completed' ORDER BY t.id DESC""")
...
def get_unpaid_bills_for_customer(phone):
    return fetch_all("""
        SELECT t.id as trip_id, t.fare_amount, t.completed_at, b.id as booking_id, b.discount_applied, b.payment_status, b.pickup_location
        FROM trips t JOIN bookings b ON t.booking_id = b.id
        WHERE b.phone = ? AND t.trip_status = 'completed' AND b.payment_status != 'paid' ORDER BY t.id DESC
    """, (phone,))
...
def get_driver_pending_collections(driver_id):
    return fetch_all("""
        SELECT t.id as trip_id, t.fare_amount, b.id as booking_id, b.customer_name, b.payment_status, b.discount_applied
        FROM trips t JOIN bookings b ON t.booking_id = b.id
...
def register_driver(name, phone, password):
    ...
    # Fix: Hash the password before storing
    hashed_password = generate_password_hash(password)
    execute("INSERT INTO drivers (full_name, phone, password) VALUES (?, ?, ?)", (name, phone, hashed_password))
...
def register_admin(username, password):
    ...
    # Fix: Hash the password before storing
    hashed_password = generate_password_hash(password)
    execute("INSERT INTO admins (username, password) VALUES (?, ?)", (username, hashed_password))
