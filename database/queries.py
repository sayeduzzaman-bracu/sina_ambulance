...
from utils.location_loader import get_coordinates_for_address
import requests
...
def get_coordinates(address):
    """
    Converts a free-text address into GPS coordinates via Google Maps.
    Returns (lat, lng) on success, or (None, None) on failure / missing key.
    """
    ...
    try:
        # Standardise search to Bangladesh for accuracy
        url = (
            f"https://maps.googleapis.com/maps/api/geocode/json"
            f"?address={requests.utils.quote(address)},+Bangladesh"
...
def get_fallback_location_coordinates(address):
    """
    Offline fallback coordinate resolver.

    Delegates to location_loader.get_coordinates_for_address() which searches
    district names (English + Bengali) then division averages against the local
    Bangladesh seed database.

    Returns (lat, lng) if any match is found, or (0.0, 0.0) so that a booking
    can still be created without hard-blocking the customer.
    """
    try:
        lat, lng = get_coordinates_for_address(address)
        if lat is not None and lng is not None:
            return lat, lng
    except Exception as e:
        print(f"Offline fallback location lookup error: {e}")
...
def create_booking(name, phone, pickup, urgency, oxygen, icu, nurse, note):
    ...
    # Offline-safe fallback if Google API is unavailable or key is not set
    if lat is None or lng is None:
        lat, lng = get_fallback_location_coordinates(pickup)
...
def cancel_booking_by_customer(booking_id):
    ...
    # Non-critical — silently skip if no trip exists yet.
    try:
        execute("""
...
def get_live_map_data():
    ...
    pending = fetch_all(
        "SELECT customer_name, urgency, pickup_lat, pickup_lng "
        "FROM bookings WHERE status = 'pending' AND pickup_lat != 0.0"
    )
